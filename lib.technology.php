...
function SetTechnologyUpgrades($typeid,$buildingid,$num) {
	...
	if (!HasReq($techtype->req_geb,$techtype->req_tech,$building->user,$tech->level+1)) 
		{ if ($debug) echo "not HasReq($techtype->req_geb,$techtype->req_tech,$building->user,$tech->level+1)<br>"; return;}
	...
	if ($debug) echo " set num=$num = max(($tech->upgradetime == 0)?0:1,min($techtype->maxlevel-$tech->level,intval($num)))<br>";
