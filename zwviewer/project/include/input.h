...
public :
	...
	bool	bKeys[256];
	char	bKeyPress[256];
	bool	bButton[3];		// mousebuttons, 3 is middle
	int		iMouseWheel;	// + is up, set to 0 at the beginning of every cShell::EventLoopStep() call
	int		iMouse[2];
	...
	unsigned char	KeyConvertSDL	(const int iVKey);
	unsigned char	GetNamedKey		(const char* szName);

	void			Reset			();
	void			KeyDown			(const unsigned char iKey);
